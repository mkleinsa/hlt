...
// [[Rcpp::export]]
double lgp2PR2D(IntegerMatrix & x,
                NumericMatrix & z,
...
// [[Rcpp::export]]
double lt2PR2D(IntegerMatrix & x,
               NumericMatrix & z,
               int iter,
               int burn,
               double delta,
  ...
  for(int it = 1; it < iter; it++) {
    ...
    for(int q = ix(3) - 1; q < ixe(3); q ++){
      oldpars(q) = abs2(oldpars(q));
      newpars(q) = abs2(newpars(q));
    }
    
    double numer = lgp2PR2D(x,
                            z,
                            ...
                            newpars[Range(ix(4) - 1, ixe(4) - 1)],
                            eps);
    
    double denom = lgp2PR2D(x,
                            z,
